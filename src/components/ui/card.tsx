...
function Card({
  ...
  return (
    <div
      ...
      data-size={size}
      className={cn(
        "group/card flex flex-col gap-(--card-spacing) overflow-hidden rounded-2xl bg-card py-(--card-spacing) text-sm text-card-foreground shadow-md ring-1 ring-foreground/8 transition-all duration-200 [--card-spacing:--spacing(8)] has-[>img:first-child]:pt-0 data-[size=sm]:[--card-spacing:--spacing(5)] *:[img:first-child]:rounded-t-2xl *:[img:last-child]:rounded-b-2xl",
        interactive && "cursor-pointer hover:shadow-lg hover:-translate-y-0.5 hover:ring-foreground/15",
        className
      )}
      {...props}
    />
